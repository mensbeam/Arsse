...
class TestV1_3 extends TestV1_2 {
    ...
    public function testChangeMarksOfMultipleArticles(string $path, string $in, array $op, Context $context): void {
        ...
        $exp = HTTP::respEmpty(204);
        $this->assertMessage($exp, $this->req("POST", $path, $in));
    ...
    public static function provideMultipleMarkings(): iterable {
        ...
        foreach ($payloads as $k => $v) {
            yield ["/items/$k/multiple", "",                                 $v, (clone $c)->editions([])];
            yield ["/items/$k/multiple", json_encode(['itemIds' => []]),     $v, (clone $c)->editions([])];
            yield ["/items/$k/multiple", json_encode(['itemIds' => "ook"]),  $v, (clone $c)->editions([])];
            yield ["/items/$k/multiple", json_encode(['itemIds' => $in[0]]), $v, (clone $c)->editions($in[0])];
            yield ["/items/$k/multiple", json_encode(['itemIds' => $in[1]]), $v, (clone $c)->editions($in[1])];
        }
    }
